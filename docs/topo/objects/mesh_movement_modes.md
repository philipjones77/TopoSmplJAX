...
This document is the canonical mode catalog for mesh movement and remeshing workflows in TopoJAX.
...
2. `remesh-restart`
   Optimize with fixed topology for one phase, stop, remesh discretely, then restart on the new topology.
...
This file is a catalog of runtime-facing workflow modes. Theory belongs in `docs/topo/theory/mesh_movement_modes.md`. Engineering mapping belongs in `docs/topo/implementation/mesh_movement_modes.md`. Mode-specific planning belongs in `docs/topo/status/`.
