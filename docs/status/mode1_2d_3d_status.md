...
- the repo virtual environment now includes the optional visualization dependencies needed by `tests/test_mode1_fixed_topology.py`
- `tests/test_mode1_fixed_topology.py` passes in the repo virtual environment
...
```powershell
c:\dev\GmshJAX\.venv\Scripts\python.exe -m pip install matplotlib>=3.8 pyvista>=0.43
$env:PYTHONPATH='src'
c:\dev\GmshJAX\.venv\Scripts\python.exe -m pytest tests/test_mode1_fixed_topology.py -vv
c:\dev\GmshJAX\.venv\Scripts\python.exe -m pytest tests/test_mode1_arbitrary_domains.py tests/test_mode1_workflow.py tests/test_m3_diagnostics_io.py -q
```
...
- add `docs/specs/` material for mesh topology classes and domain-initializer semantics that are now effectively first-class runtime objects
