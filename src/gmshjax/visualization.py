...
def build_pyvista_dataset(points: jnp.ndarray, topology: MeshTopology):
    """Build a PyVista dataset for triangles, quads, or tetrahedra."""
    try:
...
def plot_mode1_matplotlib(
    ...
    *,
    ...
    if order == 4:
        ...
        edge_faces = np.concatenate([faces[:, [0, 1]], faces[:, [1, 2]], faces[:, [2, 0]]], axis=0)
        unique_edges = np.unique(np.sort(edge_faces, axis=1), axis=0)
        lines = pts3[unique_edges]
...
def plot_mode1_viper(points: jnp.ndarray, topology: MeshTopology, *, title: str = "Mode 1 Fixed-Topology Mesh"):
    ...
    of GmshJAX. If the installed package does not expose a recognized entry point,
